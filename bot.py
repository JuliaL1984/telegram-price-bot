...
# ОБНОВЛЕНО:
# • Убраны подсказки «Добавь текст…»
# • Фото/видео/альбом без подписи ждут текст до ALBUM_WINDOW_SECONDS
# • Текст собирается несколькими сообщениями с дебаунсом TEXT_SETTLE_MS
...
# Дебаунс для склейки нескольких текстов (цена, размеры, доп. строки)
...
last_media: Dict[int, Dict[str, Any]] = {}        # по чату: ожидаем связку медиа→текст
...
def _arm_batch_timer(chat_id: int, rec: BatchRec):
    # Если медиа не придут вовремя — ничего не публикуем
    if rec.timer:
        rec.timer.cancel()
    async def _fire():
        try:
...
def extract_sizes_anywhere(text: str) -> str:
    ...
    if not (ranges_dash or ranges_slash) and not singles_alpha:
        ...
        if len(only_nums) == 1:
            ...
            f = float(val)
            if not (30.0 <= f <= 46.0 or 5.0 <= f <= 12.0 or 1.0 <= f <= 6.0):
                return ""
    return ", ".join(parts)
...
def pick_sizes_line(lines: List[str]) -> str:
    for line in lines:
        ...
        if not l or _is_price_line(l):
            continue
        if re.search(rf"\b(XXS|XS|S|M|L|XL|XXL)\b", l, flags=re.I):
            return l
...
# ----------- разрезатель блоков (по «ценовым строкам») -----------
...
def build_result_text_for_block(user_id: int, text_block: str) -> str:
    ...
    if price is None:
        ...
        parts = ["⚠️ Не нашла цену. Пример: 650€ -35% или 1360-20%"]
...
async def show_help(msg: Message):
    await msg.answer(
        ...
        "• Альбом/фото без подписи ждут текст до 30 секунд.\n"
        "• Текст можно слать несколькими сообщениями — бот склеит.\n"
...
# ====== ХЕЛПЕРЫ ОЖИДАНИЯ ТЕКСТА ДЛЯ МЕДИА ======
def _cancel_task(t: Optional[asyncio.Task]):
    if t:
        t.cancel()

async def _auto_publish_plain(chat_id: int, bucket_key: int):
    """Автопубликация медиа без цены, если текста так и не пришло к дедлайну."""
    await asyncio.sleep(ALBUM_WINDOW_SECONDS)
    bucket = last_media.get(chat_id)
    if not bucket or bucket.get("bucket_key") != bucket_key:
        return
    # если текст всё-таки накопился — публикацию сделает debounce-таймер
    if bucket.get("pending_text"):
        return
    items = bucket.get("items") or []
    first_mid = bucket.get("first_mid")
    user_id = bucket.get("user_id", 0)
    caption = bucket.get("caption", "") or ""
    del last_media[chat_id]
    await publish_to_target(first_mid=first_mid, user_id=user_id, items=items, caption=caption)

def _arm_text_debounce(chat_id: int, bucket_key: int):
    """Дебаунс для склейки нескольких текстов (240€ → 38/39 → …)."""
    bucket = last_media.get(chat_id)
    if not bucket or bucket.get("bucket_key") != bucket_key:
        return
    _cancel_task(bucket.get("debounce_task"))
    ...
    async def _fire():
        try:
            ...
            return
        b = last_media.get(chat_id)
        if not b or b.get("bucket_key") != bucket_key:
            return
        items = b.get("items") or []
        first_mid = b.get("first_mid")
        user_id = b.get("user_id", 0)
        raw_text = (b.get("caption") or "")
        if b.get("pending_text"):
            if raw_text:
                raw_text += "\n"
            raw_text += b.get("pending_text", "")
        result = build_result_text(user_id, raw_text) if raw_text else ""
        del last_media[chat_id]
        if result:
            ...
            await publish_to_target(first_mid=first_mid, user_id=user_id, items=items,
                                    caption=f"⚠️ Не нашла цену в тексте.\n\n{raw_text}".strip())
...
async def _remember_media_for_text(chat_id: int, user_id: int, items: List[Dict[str, Any]], first_mid: int, caption: str = ""):
    """Сохраняем медиа и ждём ближайший текст. Автопубликуем без цены по таймауту."""
    bucket_key = first_mid  # уникальный ключ ведра
    # отменим прежний bucket, чтобы не залипало
    old = last_media.get(chat_id)
    if old:
        _cancel_task(old.get("autopub_task"))
        _cancel_task(old.get("debounce_task"))
    last_media[chat_id] = {
        "ts": datetime.now(),
        "items": items,
        "caption": caption or "",
        ...
        "pending_text": "",
        "debounce_task": None,
        "autopub_task": asyncio.create_task(_auto_publish_plain(chat_id, bucket_key)),
...
async def handle_single_photo(msg: Message):
    ...
    # Если есть ожидающая партия (эмодзи→медиа) — прикрепляем
    if _attach_media_to_next_batch(msg.chat.id, [item], msg.from_user.id):
        return

    # Если есть подпись с ценой — сразу публикуем
    if caption:
    ...
    # Иначе — ждём текст (без подсказок)
...
async def handle_album_any(msg: Message):
    ...
    async def _flush_album():
        ...
        # НЕТ подписи у альбома: ждём ближайший текст (как для одиночных медиа)
        await _remember_media_for_text(chat_id, user_id, items, first_mid=first_mid, caption="")
...
async def handle_text(msg: Message):
    ...
    # Пересланные тексты без цены — игнор
    if not has_price and is_forward:
        return

    # Партии «текст→медиа» оставляем для админов (эмодзи-оформление)
    ...
    # Привязка к последнему медиа/альбому этого чата
    bucket = last_media.get(chat_id)
    if bucket and (datetime.now() - bucket["ts"] <= timedelta(seconds=ALBUM_WINDOW_SECONDS)):
        # накапливаем текст и перезапускаем дебаунс
        if bucket.get("pending_text"):
            bucket["pending_text"] += "\n" + txt
        else:
            bucket["pending_text"] = txt
        _arm_text_debounce(chat_id, bucket.get("bucket_key"))
        return

    # (Опционально) привязка к ещё не закрытому альбому (если вдруг подпись была, а ты дописала текст)
    ...
    # Чистые тексты без цены и без custom_emoji — как есть
